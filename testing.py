...
SEPARATOR = '*'*75
typeList = [ "md5", "bcrypt", "sha256", "sha512"]
PASSWORD_FILE = "common-passwords.txt"

# GUI code section
class STDText(object):
...
class MyApp(object):
    ...
    def InitGUI(self):
        ...
        clear_button = Button(self.root, text="Clear", command=self.clear)
        panel_bottom.add(clear_button, stretch='always')

        quit_button = Button(self.root, text="Quit", command=self.root.quit)
        panel_bottom.add(quit_button, stretch='always')

        # Redirect stdout
        self.panel = ScrolledText.ScrolledText(self.root, bg='grey', height='300')
        self.panel.pack(fill=BOTH)

        redir = STDText(self.panel)
        sys.stdout = redir
